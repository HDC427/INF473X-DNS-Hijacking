...
/*
void build_answer(uint8_t *buf, int *n){
	//modify the header section
	dns_header *dns = (dns_header*)buf;
	dns->qr = 1;
	dns->an_count = dns->qd_count;

	//start of query sections
	int q = sizeof(dns_header);
	uint8_t	*q_p = &buf[q];

	//start of answer sections
	uint8_t *a_p = buf + *n;

	//build answer sections
	for(int i=0; i<ntohs(dns->qd_count); i++){
		*a_p = 0x0c; //cf the compression rule
		a_p += 1;
		*a_p = q;
		*n += 2;

		r_element *re = (r_element*)a_p;
		...
		//rdata
		a_p += sizeof(r_element);
		*n += sizeof(r_element);
		inet_pton(AF_INET, "10.10.10.10", a_p);
		a_p += 4;
		*n += 4;
